...
func main() {
	...
	for !window.ShouldClose() {
		...
		err = context.BeginLayout()
		if err != nil {
			log.Fatalf("BeginLayout error: %v", err)
		}
		...
		err = context.ConfigureOpenElement(glay.ElementDeclaration{
		...
		err = context.OpenElement()
		if err != nil {
			log.Fatalf("Failed to open button 1: %v", err)
		}

		err = context.ConfigureOpenElement(glay.ElementDeclaration{
			ID:              glay.ID("Button1"),
			BackgroundColor: glay.Color{255, 120, 200, 255},
			Layout: glay.LayoutConfig{
				Sizing: glay.Sizing{
					Width:  glay.NewSizingAxis(glay.SizingFixed, 200),
					Height: glay.NewSizingAxis(glay.SizingFixed, 50),
				},
			},
			Floating: glay.FloatingElementConfig{
		...
		if err != nil {
			log.Fatalf("Failed to configure button 1: %v", err)
		}

		err = context.CloseElement()
		if err != nil {
			log.Fatalf("Failed to close button 1: %v", err)
		}
		...
		err = context.OpenElement()
		if err != nil {
			log.Fatalf("Failed to open button 2: %v", err)
		}

		err = context.ConfigureOpenElement(glay.ElementDeclaration{
			ID:              glay.ID("Button2"),
			BackgroundColor: glay.Color{100, 120, 200, 255},
			Layout: glay.LayoutConfig{
				Sizing: glay.Sizing{
					Width:  glay.NewSizingAxis(glay.SizingFixed, 200),
					Height: glay.NewSizingAxis(glay.SizingFixed, 50),
		...
		if err != nil {
			log.Fatalf("Failed to configure button 2: %v", err)
		}

		err = context.CloseElement()
		if err != nil {
			log.Fatalf("Failed to close button 2: %v", err)
		}
